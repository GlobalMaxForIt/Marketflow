...
let open_reserve_price_input = document.getElementById('open_reserve_price_input')
...
let reserve_address_ = document.getElementById('reserve_address')
let reserve_region_ = document.getElementById('reserve_region')
let reserve_district_ = document.getElementById('reserve_district')

let reserve_modal_one = document.querySelector('#reserve_modal_one')
let orderData = []
...
let clientId = 0
let percentV = 0
function getOrderAllSum(left_percent, discountPercent){
    console.log([discountPercent, left_percent])
    if(discountPercent>0){
        percent_v = left_percent
        open_reserve_price_input.value = total_all_left_sum
    }else{
        percent_v = 0
        hideClientDiscount()
    }
    confirm_client_discount_func(discountPercent)
}
            ...
            try{
                $.ajax({
                    ...
                    data:{
                        ...
                        'address':reserve_address_.value,
                        'region':reserve_region_.value,
                        'district':reserve_district_.value,
                    },
...
reserve_modal_one.addEventListener('scroll', function () {
    if(client_name.value == '' || client_surname.value == '' || client_phone.value == ''){
        client_create_submit.disabled = true
    }else{
...
let reserve_loader = document.getElementById("reserve_loader")
let reserve_content_info = document.getElementById("reserve_content_info")
...
client_create_submit.addEventListener('click', function (e) {
    e.preventDefault()
    if(open_reserve_price_input.hasAttribute('max')){
        open_reserve_price_input.removeAttribute('max')
    }
    open_reserve_price_input.value = 0
    debt_two_client_id.value = ''
    if(reserve_loader.classList.contains("d-none")){
        reserve_loader.classList.remove("d-none")
    }
    if(!reserve_content_info.classList.contains("d-none")){
        reserve_content_info.classList.add("d-none")
    }
    client_info = {
    ...
    setTimeout(function () {
        if(client_id__ != ''){
            $(document).ready(function () {
                $.ajax({
                    url:`/../api/get-client-by-id/${client_id__}`,
                    type:'GET',
                    success: function (data) {
                        if(data.status == 200){
                            ...
                            if(Object.keys(client_data).length>0){
                                ...
                                if(total_all_left_sum>0){
                                    open_reserve_price_input.setAttribute('max', total_all_left_sum)
                                    open_reserve_price_input.value = total_all_left_sum
                                }
                            }else{
                            ...
                            if(!reserve_loader.classList.contains("d-none")){
                                reserve_loader.classList.add("d-none")
                            }
                            if(reserve_content_info.classList.contains("d-none")){
                                reserve_content_info.classList.remove("d-none")
                            }
                        }
                    }
                })
        ...
        $.ajax({
            ...
            success: function (data) {
                if(data.status == 200){
                    ...
                    console.log(client_data_)
                    if(Object.keys(client_data_).length>0){
                        left_percent = parseFloat(data.discount)
                        discountPercent = parseInt(data.discount_percent)
                        getOrderAllSum(left_percent, discountPercent)
                        debitor_client_full_name_html.textContent = client_data_.fullname
                        debt_two_client_id.value = client_data_.id
                        setClientInfoForDebt(client_data_)
