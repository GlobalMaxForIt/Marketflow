...
function getOrderAllSum(left_percent, discountPercent){
    if(discountPercent>0){
        ...
        open_delivery_price_input.value = total_all_left_sum
    }else{
        percent_v = 0
        hideClientDiscount()
    }
...
function orderDelivery() {
    ...
    $(document).ready(function () {
        if(order_data.length>0 && debt_two_client_id.value){
            try{
                $.ajax({
                        ...
                        if(data.status == 200){
                            ...
                            if(Object.keys(client_data).length>0){
                                ...
                                if(total_all_left_sum>0){
                                    open_delivery_price_input.setAttribute('max', total_all_left_sum)
                                    open_delivery_price_input.value = total_all_left_sum
                                }
                            }else{
