...
@section('content')
            ...
            <ul class="nav nav-tabs mb-4" id="myTab" role="tablist">
                ...
                <li class="nav-item ms-2">
                    <a href="#discount_client" data-bs-toggle="tab" aria-expanded="false" class="nav-link">
                        {{translate_title("Clients' discount")}}
            ...
            <div class="card-body">
                <div class="tab-content" id="discount_tab">
                    <div class="tab-pane fade show active" id="discount" role="tabpanel" aria-labelledby="discount-tab">
                        <div class="card">
                            <div class="right_button_create">
                                <button class="form_functions global-button" data-bs-toggle="modal" data-bs-target="#create_modal" data-url="{{route('discount.store')}}">
                                    <img src="{{asset('menubar/discount_active.png')}}" alt="" height="20px">
                                    {{translate_title('Новый Скидка')}}
                                </button>
                            </div>
                            <div class="card-body">
                                <table class="datatable restaurant_tables table table-striped table-bordered dt-responsive nowrap">
                                    <thead>
                                        <tr>
                                            <th>#</th>
                                            <th>{{translate_title('Discount percent')}}</th>
                                            <th>{{translate_title('Category')}}</th>
                                            <th>{{translate_title('Product')}}</th>
                                            <th>{{translate_title('Number of products')}}</th>
                                            <th class="text-center">{{translate_title('Functions')}}</th>
                                        </tr>
                                    </thead>
                                        ...
                                        <tr>
                                            ...
                                            <td class="function_column">
                                                <div class="d-flex justify-content-around align-items-center height_50 function_buttons">
                                                    <a class="edit_button btn" href="{{route('discount.edit', $discount_data['discount'][0]->id)}}">
                                                        <img src="{{asset('img/edit_icon.png')}}" alt="" height="18px">
                                                    </a>
                                                    <button type="button" class="btn delete_button btn-sm waves-effect" data-bs-toggle="modal" data-bs-target="#delete_modal" data-url="{{route('discount.destroy', $discount_data['discount'][0]->id)}}">
                    ...
                    <div class="tab-pane fade" id="discount_client" role="tabpanel" aria-labelledby="discount_client-tab">
                        <div class="card">
                            <div class="right_button_create">
                                <button class="form_functions global-button" data-bs-toggle="modal" data-bs-target="#create_discount_client_modal" data-url="{{route('discount.store')}}">
                                    <img src="{{asset('menubar/discount_active.png')}}" alt="" height="20px">
                                    {{translate_title('Новый Скидка')}}
                                </button>
                            </div>
                                ...
                                <table class="table datatable restaurant_tables table-striped table-bordered dt-responsive nowrap">
                                    <thead>
                                        <tr>
                                            <th>#</th>
                                            <th>{{translate_title('Discount percent')}}</th>
                                            <th>{{translate_title('Client')}}</th>
                                            <th>{{translate_title('Discount number')}}</th>
                                            <th class="text-center">{{translate_title('Functions')}}</th>
                                        </tr>
                                    </thead>
                                    <tbody class="table_body">
                                    ...
                                    @foreach($discounts_client_data as $discount_client_data)
                                        ...
                                        <tr>
                                            ...
                                            <td class="function_column">
                                                <div class="d-flex justify-content-around align-items-center height_50 function_buttons">
                                                    <a class="edit_button btn" href="{{route('discount.edit', $discount_client_data['id'])}}">
                                                        <img src="{{asset('img/edit_icon.png')}}" alt="" height="18px">
                                                    </a>
                                                    <button type="button" class="btn delete_button btn-sm waves-effect" data-bs-toggle="modal" data-bs-target="#delete_modal" data-url="{{route('discount.destroy', $discount_client_data['id'])}}">
                                                        <img src="{{asset('img/trash_icon.png')}}" alt="" height="18px">
                                                    </button>
                                                </div>
                                            </td>
        ...
        <div class="modal-dialog modal-dialog-scrollable" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="scrollableModalTitle">{{translate_title('New discount')}}</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <form action="{{route('discount.store')}}" class="modal-body needs-validation" method="POST" novalidate>
                    @csrf
                    @method("POST")

                    <div class="position-relative mb-3">
                        <label class="form-label">{{translate_title('Discount percent')}}</label>
                    ...
                    <div class="position-relative mb-3">
                        <select class="form-select" name="products_categories_id" id="category_id" aria-label="Floating label select example" required>
                            ...
                            <option value="all">{{translate_title('All subcategory')}}</option>
                        ...
                        <label class="form-label" for="category_id">{{translate_title('Products categories')}}</label>
                    </div>
                    ...
                    <div class="position-relative mb-3 d-none" id="product_exists">
                        <div class="form-floating">
                            ...
                            <label for="subcategory_id">{{translate_title('Products')}}</label>
                        </div>
                    </div>
                    <div class="position-relative mb-3">
                        <label class="form-label">{{translate_title('Range date')}}</label>
                        ...
                        <div class="invalid-tooltip">
                            {{translate_title('Please enter range date.')}}
                        </div>
                    </div>
    ...
    <div class="modal fade" tabindex="-1" role="dialog" id="create_discount_client_modal"
        ...
        <div class="modal-dialog modal-dialog-scrollable" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="scrollableModalTitle">{{translate_title('New clients\' discount')}}</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <form action="{{route('discount.store')}}" class="modal-body needs-validation" method="POST" novalidate>
                    @csrf
                    ...
                    <div class="position-relative mb-3">
                        ...
                        <div class="invalid-tooltip">
                            {{translate_title('Please select client.')}}
                        </div>
                        <label class="form-label" for="client_select_id">{{translate_title('Clients')}}</label>
                    </div>
                    <div class="position-relative mb-3">
                        <label class="form-label">{{translate_title('Range date')}}</label>
                        <input type="text" name="start_end_date" class="form-control range-datepicker" value="{{old('start_end_date')}}" placeholder="2018-10-03 to 2018-10-10" required>
    ...
    <script>
        ...
        let text_select_sub_category = "{{translate_title('Select sub category')}}"
        let text_all_subcategory_products = "{{translate_title('All subcategories`s products')}}"
        let text_all_products = "{{translate_title('All products')}}"
        let text_select_product = "{{translate_title('Select product')}}"
