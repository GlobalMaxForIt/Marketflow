...
@section('content')
        ...
        <div class="order-section">
            <div class="card">
                <div class="card-header">
                    <h4 class="mt-0 header-title">{{translate_title('Edit company', $lang)}}</h4>
                </div>
                <div class="card-body">
                    <form class="modal-body needs-validation" action="{{route('organizations.update', $organization['id'])}}" method="POST" enctype="multipart/form-data" novalidate>
                        @csrf
                        @method('PUT')
                        <div class="row">
                            <div class="position-relative col-6 mb-3">
                                <label for="name" class="form-label">{{translate_title('Name', $lang)}}</label>
                                <input type="text" id="name" class="form-control" name="name" value="{{$organization['name']}}" required>
                        ...
                        <div class="row">
                            <div class="col-6 mb-3">
                                ...
                                <input type="text" id="address" class="form-control" name="address" value="{{$organization['address']?$organization['address']->name:''}}">
                            </div>
                        </div>
    ...
    <script>
        ...
        @if($organization['address'])
            @if($organization['address']->cities)
                let current_region = "{{$organization['address']->cities->region?$organization['address']->cities->region->id:''}}"
                let current_district = "{{$organization['address']->cities->id??''}}"
            @else
                let current_region = ''
