                    ...
                    <div class="tab-pane fade show active" id="employees" role="tabpanel" aria-labelledby="employees-tab">
                        <div class="card">
                            <div class="right_button_create">
                                <a class="form_functions global-button" data-bs-toggle="modal" data-bs-target="#create_modal" data-url="{{route('organizations.store')}}">
                                    <span class="mdi mdi-basket-fill font-20"></span>
                                    {{translate_title('New organization', $lang)}}
                                </a>
                            </div>
                            <div class="card-body overflow-auto">
                                <table id="datatable-buttons" class="restaurant_tables table table-striped table-bordered dt-responsive nowrap">
                                    ...
                                    @foreach($organizations as $organization)
                                        <tr>
                                            <td class="show_page">{{$organization['id']}}</h6></td>
                                            <td><h6>{{$organization['name']}}</h6></td>
                                            <td><h6>{{$organization['address']}}</h6></td>
                                            <td>
                                                <a class="product_images_column" onclick='getImages("{{implode(" ", $organization['images'])}}")' data-bs-toggle="modal" data-bs-target="#carousel-modal">
                                            ...
                                            <td>
                                                <div class="d-flex justify-content-around align-items-center height_50 function_buttons">
                                                    <a class="edit_button btn me-2" href="{{route('organizations.edit', $organization['id'])}}">
                                                        <img src="{{asset('img/edit_icon.png')}}" alt="" height="18px">
                                                    </a>
                                                    <a type="button" class="btn delete_button btn-sm waves-effect" data-bs-toggle="modal" data-bs-target="#delete_modal" data-url="{{route('organizations.destroy', $organization['id'])}}">
        ...
        <div class="modal-dialog modal-dialog-scrollable" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="scrollableModalTitle">{{translate_title('New organization', $lang)}}</h5>
                    <a type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></a>
                </div>
                <form class="modal-body needs-validation" action="{{route('organizations.store')}}" method="POST" enctype="multipart/form-data" novalidate>
                    @csrf
                    @method('POST')
                    <div class="position-relative mb-3">
                        ...
                        <div class="invalid-tooltip">
                            {{translate_title('Please enter stuff category.', $lang)}}
                        </div>
                    </div>
                    <div class="mb-3">
